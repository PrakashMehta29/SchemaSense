        ...
        <div className="flex items-center gap-1 mt-3 border-t border-border/20 pt-2.5">
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom" className="text-[10px] p-1.5 bg-background border text-foreground">
              {copied ? "Copy Response" : "Copy Response"}
            </TooltipContent>
          </Tooltip>
          ...
          <div className="ml-1 pl-1 border-l border-border/20">
            <SpeakResponseButton text={text} messageId={messageId} />
          </div>
        </div>

        {/* Voice Playback Controls (renders inline when active speaking) */}
        <VoicePlaybackControls messageId={messageId} />
      </div>
    </TooltipProvider>
  );
}
