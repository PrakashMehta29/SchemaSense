...
export function VoiceInputButton({ disabled }: { disabled?: boolean }) {
  ...
  let btnClasses = "border-border/60 bg-background text-muted-foreground hover:text-foreground hover:bg-secondary";
  if (isRecording) {
    btnClasses = "border-red-500/30 bg-red-500/15 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.25)] animate-pulse";
  } else if (isListening) {
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            aria-label={
              isRecording ? "Stop voice recording" : isTranscribing ? "Transcribing speech" : "Start voice input"
            }
          >
            {isTranscribing ? (
        ...
        <TooltipContent side="top" className="text-xs p-2 bg-background border text-foreground rounded-lg shadow-md z-55">
          {isRecording
