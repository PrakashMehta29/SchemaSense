...
export function DatasetCard({ dataset, isActive }: DatasetCardProps) {
  ...
  const getHealthColor = (score: number) => {
    if (score >= 90)
      return "text-emerald-500 border-emerald-500/20 bg-emerald-500/10 dark:bg-emerald-500/5";
    if (score >= 80)
  ...
  return (
    ...
    >
      ...
      {isActive && <div className="absolute top-0 left-0 w-full h-[1.5px] bg-primary glow-lime" />}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <span className="text-[10px] text-muted-foreground/80 font-mono-tight">
              Rows:{" "}
              <span className="text-foreground font-semibold">{dataset.rows || "Unknown"}</span> ·
              Size: {dataset.size}
            </span>
          </div>
