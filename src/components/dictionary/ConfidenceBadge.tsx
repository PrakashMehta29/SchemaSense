  ...
  const getBadgeStyle = () => {
    if (score >= 95) {
      return {
        ...
        icon: CheckCircle2,
        tooltip: "AI generated this description with high accuracy based on type matching and complete records analysis.",
      };
    }
    if (score >= 80) {
      return {
        ...
        icon: AlertTriangle,
        tooltip: "AI processed this field with minor assumptions. Review the business meaning context.",
      };
    }
    return {
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent side="top" className="max-w-[200px] text-[11px] leading-normal p-2.5 bg-background border border-border/80 text-foreground shadow-lg backdrop-blur">
          {tooltip}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
