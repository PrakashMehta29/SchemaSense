...
export function VoiceInputCard() {
    ...
    if ("speechSynthesis" in window) {
      ...
      if (voices.length > 0) {
        if (langCode.startsWith("en")) {
          ...
          const googleLang = voices.find(
            (v) => v.name.includes("Google") && v.lang.startsWith(langCode.split("-")[0]),
          );
  ...
  const handleSend = () => {
    ...
    const selectedOption =
      LANGUAGE_OPTIONS.find((opt) => opt.code === selectedLang) || LANGUAGE_OPTIONS[0];
          ...
          <div className="flex flex-col gap-6 w-full max-w-md pt-8">
            ...
            <div className="flex items-center gap-3 bg-card border shadow-lg rounded-full px-4 h-14 w-full animate-in slide-in-from-bottom-4 duration-300">
              <Button
                ...
                className="text-destructive hover:bg-destructive/10 hover:text-destructive shrink-0 rounded-full w-10 h-10"
              >
              ...
              {state === "recording" ? (
                <Button
                  ...
                  className="text-primary hover:bg-primary/10 shrink-0 rounded-full w-10 h-10"
                >
              ...
              <Button
                size="icon"
                onClick={handleSend}
                className="bg-primary text-primary-foreground shrink-0 rounded-full w-10 h-10 shadow-md hover:scale-105 transition-transform"
              >
        ...
        <div className="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="flex flex-col gap-6">
            {history.map((item, index) => (
              <GlassCard
                key={item.id}
                className={`p-6 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500`}
