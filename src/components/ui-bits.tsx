...
export function SectionTitle({
  ...
  return (
    <div className="mb-8 relative overflow-hidden rounded-2xl border glass-panel p-6 md:p-8 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.15)]">
      ...
      <h1 className="font-display text-3xl font-bold leading-tight md:text-4xl text-foreground">
        {title}
      </h1>
...
export function GlassCard({
  ...
  return (
    <div
      className={`relative rounded-2xl border glass-panel-heavy backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.15)] ${className}`}
    >
      <div className="absolute top-0 left-0 w-full h-[1px] glass-edge" />
      {children}
    </div>
...
export function Pill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 font-mono-tight text-[10px] uppercase tracking-wider ${styles}`}
    >
      {children}
    </span>
  );
