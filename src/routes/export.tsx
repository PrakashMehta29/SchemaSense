  ...
  return (
    <div>
      <SectionTitle
        ...
        title={
          <>
            Ship it <span className="text-primary">anywhere.</span>
          </>
        }
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {formats.map((f, i) => {
          ...
          return (
            ...
            >
              <GlassCard
                className={`group relative overflow-hidden p-6 transition-colors ${isProcessing || isCompleted ? "border-primary/50" : "hover:border-primary/50"}`}
              >
                <div
                  className={`pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full blur-3xl transition-all ${isProcessing || isCompleted ? "bg-primary/25" : "bg-primary/0 group-hover:bg-primary/25"}`}
                />
                <div className="flex items-center justify-between">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-xl border transition-colors ${isProcessing || isCompleted ? "border-primary bg-primary/20 text-primary" : "border-primary/40 bg-primary/10 text-primary"}`}
                  >
