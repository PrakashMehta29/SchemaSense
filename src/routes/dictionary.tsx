...
import { generateMetadataFn } from "@/lib/api/schemaSense";
  ...
  head: () => ({
    meta: [
      { title: "Dictionary · SchemaSense" },
      {
        name: "description",
        content:
          "AI-powered definitions, types, business meaning, and confidence for every column.",
      },
    ],
  }),
...
function Dictionary() {
    ...
    if (generatedFlag === "1") {
      ...
      if (savedMeta) {
        try {
          ...
          console.warn("Could not load dictionary metadata:", err);
        }
      }
    }

    // Handle auto-generation trigger from upload screen
    const autoGenFlag = localStorage.getItem("schema_sense_auto_generate");
    if (autoGenFlag === "1") {
      localStorage.removeItem("schema_sense_auto_generate");
      setIsModalOpen(true);
    }
  ...
  const handleGenerationComplete = async () => {
    ...
    try {
      const resp = await generateMetadataFn({
        data: {
      ...
      if (resp && resp.success && resp.results) {
        const newMeta: Record<string, ColMetadata> = {};
        const govAssetsToSave: Record<string, unknown>[] = [];

        resp.results.forEach((item) => {
          const colTypeObj = columns.find((c) => c.name === item.columnName);
          const colType = colTypeObj ? colTypeObj.type : "string";

          let samples = [
            item.columnName.toUpperCase() + "_902",
            item.columnName.toUpperCase() + "_104",
          ];
          if (item.piiType !== "None") {
            if (item.piiType === "Email") {
              samples = ["user@example.com", "customer@domain.com"];
            } else if (item.piiType === "Phone Number") {
              samples = ["+1-555-0199", "+1-555-0143"];
            } else {
          ...
          newMeta[item.columnName] = {
            name: item.columnName,
            type: colType,
            nullPct: Math.random() > 0.5 ? "2.4%" : "0.0%",
            uniquePct: item.columnName.toLowerCase().includes("id") ? "100%" : "85.4%",
            description: item.description,
            meaning: item.businessMeaning,
            context: "Used across analytical pipelines, reporting, and database views.",
            samples,
            relations: [],
            isFk: false,
            sensitivity,
            pii: item.piiType !== "None",
          ...
          govAssetsToSave.push({
            ...
            riskReason: item.piiReason || item.description,
            piiType: item.piiType,
            piiReason: item.piiReason,
            confidence: item.confidence,
          });
        });
        ...
        try {
          ...
          // Refresh workspace states if needed
          if (refreshWorkspace) {
            refreshWorkspace();
          }
        } catch (err) {
      ...
      console.error("Failed to generate model metadata, falling back to heuristics", err);
    } finally {
    ...
    return (
      <div className="py-12 space-y-8">
        <SectionTitle
          ...
          title={
            <>
              Data <span className="text-primary">Dictionary.</span>
            </>
          }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <GlassCard className="p-4 flex flex-col justify-between min-h-[90px] border-border/40 relative overflow-hidden">
          ...
          <span className="font-display text-2xl font-bold text-foreground">{columns.length}</span>
        </GlassCard>
        ...
        <GlassCard className="p-4 flex flex-col justify-between min-h-[90px] border-border/40 relative overflow-hidden">
          ...
          <span
            className={`font-display text-sm font-bold flex items-center gap-1.5 ${isGenerated ? "text-emerald-500" : "text-amber-500 animate-pulse"}`}
          >
            {isGenerated ? (
              <>
        ...
        {isGenerating ? (
          ...
          filteredColumns.map((col) => {
            ...
            const meta = metadata[col.name] || {
              ...
              description:
                "AI description is pending audit. Please trigger 'Generate Metadata' to start automated catalog profiling.",
