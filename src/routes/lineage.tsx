  ...
  head: () => ({
    meta: [
      { title: "Lineage · SchemaSense" },
      {
        name: "description",
        content:
          "Interactive node-graph visualising the full data lineage from raw sources to marts.",
      },
    ],
  }),
  component: Lineage,
...
const DEFAULT_NODES: NodeDef[] = [
  // RAW
  {
    ...
    label: "raw.users",
    layer: "raw",
    cols: ["id", "email", "signup_ts", "country"],
    rowCount: "2.1M",
  },
  {
    id: "raw_events",
  ...
  },
  {
    ...
    label: "raw.payments",
    layer: "raw",
    cols: ["user_id", "amount", "currency", "ts"],
    rowCount: "4.7M",
  },
  // STAGING
  {
    ...
    label: "stg_customers",
    layer: "staging",
    cols: ["cust_id", "email", "country", "signup_ts"],
    rowCount: "2.1M",
  },
  {
    id: "stg_orders",
    label: "stg_orders",
    layer: "staging",
    cols: ["order_id", "cust_id", "ts", "status"],
    rowCount: "11.2M",
  },
  {
    id: "stg_revenue",
  ...
  // MART
  {
    ...
    label: "dim_customers",
    layer: "mart",
    cols: ["cust_id", "email", "country", "plan_tier"],
    rowCount: "2.1M",
  },
  {
    id: "fact_sales",
    label: "fact_sales",
    layer: "mart",
    cols: ["order_id", "cust_id", "revenue_usd", "ts"],
    rowCount: "11.2M",
  },
];
...
function Lineage() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedCols) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          const hasDefaults = parsed.some(
            (c: { name: string }) => c.name === "signup_ts" || c.name === "plan_tier",
          );
          if (!hasDefaults) {
      ...
      console.warn("Could not read lineage state:", err);
    }
  }, []);
  ...
  const handleGenerateLineage = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.warn("Could not generate lineage:", err);
      }
  ...
  const isNodeHighlighted = (id: string) => selected === null || connectedIds.has(id);
    ...
    return (
      <div className="py-12 space-y-8">
        <SectionTitle
          ...
          title={
            <>
              Data <span className="text-primary">flow.</span>
            </>
          }
      ...
      <div className="py-12">
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-card/25 border border-border/40 rounded-3xl animate-pulse">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Tracing schema references and foreign key lineages.
          </p>
        </div>
      </div>
    );
            ...
            >
                ...
                style={{
                  borderColor: isSelected
                    ...
                    : highlighted && selected
                      ? `${meta.colour}60`
                      : `${meta.colour}30`,
                  background: isSelected ? `rgba(232,97,63,0.15)` : `var(--card)`,
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `radial-gradient(circle at 50% 0%, ${meta.colour}10, transparent 70%)`,
                  }}
                />
              </div>
            </motion.button>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                >
                  <Table2
                    className="h-4 w-4"
                    style={{ color: LAYER_META[selectedNode.layer].colour }}
                  />
                </div>
                <div>
                  <div className="font-mono-tight text-sm font-bold text-foreground">
                    {selectedNode.label}
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span
                      className={`inline-flex items-center rounded-full border px-2 py-0.5 font-mono-tight text-[9px] uppercase tracking-wider ${LAYER_META[selectedNode.layer].badge}`}
                    >
                      {selectedNode.layer}
                    </span>
                    <span className="font-mono-tight text-[11px] text-muted-foreground">
                      {selectedNode.rowCount} rows
                    </span>
                  </div>
                </div>
              </div>
