...
import { AlertTriangle, Info, Database, ChevronDown, ChevronUp, Activity, BarChart2 } from "lucide-react";
...
const defaultColumns: ColumnData[] = [
  {
    name: "cust_id",
    type: "uuid",
    min: "0001bc5a", max: "fffe402a", unique: 1840022, nulls: 0,
    dist: [8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8],
  },
  {
    name: "email",
    type: "email",
    min: "a@company.com", max: "z@domain.io", unique: 1540321, nulls: 2.1,
...
function GlobalAlertBanner({ anomalies }: { anomalies: any[] }) {
    ...
    <div className="space-y-2.5">
      ...
      <div className="mt-4 rounded-xl border border-red-900/40 bg-rose-950 p-4 font-mono text-[11px] text-rose-100 shadow-inner leading-relaxed">
        <div className="flex items-center justify-between border-b border-red-900/30 pb-1.5 mb-2">
          <span className="text-[10px] font-bold uppercase tracking-wider text-rose-300">Upstream Incident Trace</span>
          <span className="text-[9px] text-rose-300/80">{failureTrace.timestamp}</span>
        </div>
        <div className="space-y-1">
          <div><span className="text-rose-400 font-bold">[CODE]:</span> {failureTrace.errorCode}</div>
          <div className="break-all"><span className="text-rose-400 font-bold">[SOURCE]:</span> {failureTrace.failedComponent}</div>
          <div><span className="text-rose-400 font-bold">[AFFECTED DOWNSTREAM]:</span> {failureTrace.downstreamImpact.join(", ")}</div>
        </div>
      </div>
    </div>
  );
...
function ContextAwareSummaryCard({
  ...
  const isText = type.includes("string") || type.includes("enum") || type.includes("email") || type.includes("uuid");
  ...
  return (
    <GlassCard className={`p-5 transition-all duration-300 ${hasAnomaly ? "border-destructive/30 shadow-[0_4px_20px_rgba(239,68,68,0.03)]" : ""}`}>
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="font-mono-tight text-[11px] text-muted-foreground mt-0.5">{column.type}</div>
        </div>
          ...
          >
            {(() => {
              if (type === "enum" || type === "boolean") {
                const categories = name === "country"
                  ? [{ label: "US", pct: 54 }, { label: "UK", pct: 28 }, { label: "DE", pct: 18 }]
                  : (name === "is_active"
                    ? [{ label: "true", pct: 64 }, { label: "false", pct: 36 }]
                    : (name === "plan_tier"
                      ? [{ label: "premium", pct: 45 }, { label: "basic", pct: 39 }, { label: "free", pct: 16 }]
                      : [{ label: "Value A", pct: 50 }, { label: "Value B", pct: 30 }, { label: "Value C", pct: 20 }]
                    )
                  );
                    ...
                    <div className="relative mt-2">
                      ...
                      <div className="flex h-16 items-end justify-around gap-6 border-b border-border/20 pb-1 px-4">
                        {categories.map((cat, i) => (
                          <div key={cat.label} className="flex-1 flex flex-col items-center h-full justify-end max-w-[80px]">
                            <motion.div
                              initial={{ height: 0 }}
                ...
                return (
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[9px] font-mono-tight uppercase tracking-wider text-muted-foreground border-t border-border/10 pt-2 mt-2">
                      <div>Median: <span className="font-bold text-foreground">{median.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
                      <div>P90: <span className="font-bold text-foreground">{p90.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
                      <div>P99: <span className="font-bold text-foreground">{p99.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
                    </div>
                  </div>
                );
              }
                ...
                return (
                  <div className="rounded-xl border border-primary/10 bg-primary/5 p-4 space-y-2.5">
                    ...
                    <div className="flex justify-between font-mono-tight text-[9px] text-muted-foreground">
                      <span>📅 MIN: <span className="text-foreground font-bold">{formattedMin}</span></span>
                      <span>📅 MAX: <span className="text-foreground font-bold">{formattedMax}</span></span>
                    </div>
                  </div>
                );
              }
...
function Quality() {
  ...
  const [anomalies, setAnomalies] = useState<any[]>(defaultAnomalies);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const richCols = parsed.slice(0, 8).map((p: any, i: number) => {
            ...
            else if (typeLower === 'number' || typeLower === 'integer' || typeLower.includes('decimal')) inferredType = 'numeric';
            ...
            return {
              name: p.name,
              type: inferredType,
              min: p.name === 'cust_id' ? '0001bc5a' : (p.name === 'email' ? 'a@company.com' : (i === 4 ? 0 : 'A')),
              max: p.name === 'cust_id' ? 'fffe402a' : (p.name === 'email' ? 'z@domain.io' : (i === 4 ? 18420 : 'Z')),
              unique: p.name === 'cust_id' ? 1840022 : (p.name === 'email' ? 1540321 : Math.floor(Math.random() * 10000) + 1),
          ...
          const errorTemplates = [
            { t: "Type mismatch", d: "rows ingested as incorrect type instead of expected format.", sev: "high" },
            { t: "Duplicate keys", d: "duplicated values detected bypassing unique constraint.", sev: "med" },
            { t: "Null spike", d: "Null rate jumped significantly over the last 30 minutes.", sev: "high" },
            { t: "Format drift", d: "% of incoming records fail the canonical regex format.", sev: "low" },
            { t: "Schema drift", d: "unexpected new data shape detected upstream.", sev: "med" },
          ];

          const mappedAnomalies = parsed.slice(0, anomalyCount).map((p: any, i: number) => {
          ...
          const avgNull = richCols.reduce((acc: number, c: any) => acc + c.nulls, 0) / richCols.length;
          const uniqueKeysCount = richCols.filter((c: any) => c.unique > 1000000 || c.name.toLowerCase().includes("id") || c.name.toLowerCase().includes("email")).length;
          const typeCollisions = (hash % 3) + 1;
          const rowsProfiled = 50000 + (hash * 12345) % 4500000;

          setMetrics([
            { l: "Avg null %", v: parseFloat(avgNull.toFixed(1)), s: "%" },
            { l: "Primary & unique keys", v: uniqueKeysCount || 2, s: "", subtitle: `Out of ${richCols.length} total columns`, decimals: 0 },
  ...
  if (!hasDataset) {
    return (
      <div className="py-12">
        <SectionTitle
          kicker="Quality"
          title={<>Schema Profiling & <span className="text-primary">Health.</span></>}
  ...
  return (
    <div>
      <SectionTitle
        ...
        title={<>Profiling, <span className="text-primary">simplified.</span></>}
            ...
            <GlassCard key={s.l} className="p-4 relative">
              ...
              {hasTooltip && activeTooltip === s.l && (
                <div className="absolute bottom-full left-0 right-0 z-50 mb-2 rounded-xl border border-border/80 bg-background/95 p-3.5 shadow-xl backdrop-blur-md">
                  <div className="text-[11px] font-bold text-foreground mb-1 leading-snug">Type Collisions</div>
                  <div className="text-[10px] text-muted-foreground leading-normal">
                    When a single column contains mixed formats or types of data (e.g. text mixed with numbers). This breaks database operations.
                  </div>
                </div>
              )}
            </GlassCard>
              ...
              <div className="px-5 pb-5 border-t border-amber-500/20 pt-4 space-y-4">
                ...
                {anomalousColumns.length > 0 && (
                  <div className="space-y-3 pt-3 border-t border-amber-500/15">
                    <h4 className="font-mono-tight text-[10px] uppercase text-muted-foreground">Affected Columns</h4>
