...
export const Route = createFileRoute("/dataset-overview")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review your uploaded dataset before generating AI metadata.",
      },
    ],
  }),
...
function DatasetOverviewPage() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Could not read overview details from storage:", err);
    }
      ...
      >
        ...
        <div>
          <p className="text-sm font-semibold text-foreground">
            Analysis Readiness: <span className="text-emerald-500">Ready</span>
          </p>
          <p className="text-[10px] text-muted-foreground mt-0.5">
            Dataset ingested and indexed. AI metadata generation is available.
          </p>
        </div>
        <div className="ml-auto">
          <span className="font-mono-tight text-[9px] uppercase tracking-widest text-muted-foreground">
            Status
          </span>
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-[10px] font-mono-tight text-emerald-500 font-semibold">
              Active
            </span>
          </div>
        </div>
          ...
          {[
            {
              ...
              desc: "Schema structure profiling and type inference",
            },
            {
              icon: Columns3,
              label: "Detecting Data Types",
              desc: "UUID, timestamps, enums, decimals auto-tagged",
            },
            {
              icon: FileSpreadsheet,
              label: "Generating Definitions",
              desc: "AI-written column descriptions and meanings",
            },
            {
              icon: ShieldCheck,
              label: "Running Governance Scan",
              desc: "PII detection, GDPR, CCPA, SOC2 flags",
            },
          ].map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className="flex items-start gap-3 p-3 rounded-xl border border-border/30 bg-secondary/10"
              >
