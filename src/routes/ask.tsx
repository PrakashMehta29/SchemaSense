...
export const Route = createFileRoute("/ask")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Chat with your schema in real time. Ask about columns, lineage, drift and more." },
    ],
  }),
  component: Ask,
});
    ...
    >
      ...
      >
        ...
        {msg.hasCode && msg.code && (
          <CodeBlock code={msg.code} lang={msg.codeLang} />
        )}
      </div>
    </motion.div>
  );
...
function Ask() {
  ...
  const isVoiceEnabled = typeof document !== "undefined"
  ...
  const send = useCallback((text: string) => {
    ...
    setTimeout(() => {
      setTyping(false);
      const reply = { ...AI_REPLIES[replyIdx.current % AI_REPLIES.length], id: crypto.randomUUID() };
      replyIdx.current += 1;
        ...
        <div className="border-t border-border/60 p-4 space-y-3">
          ...
          >
            ...
            <input
              ...
              placeholder={listening ? "Listening…" : 'Ask: "Find PII columns" or "Explain churn_risk"'}
          ...
          <p className="font-mono-tight text-[10px] text-muted-foreground/50 text-center">
            Press <kbd className="rounded border border-border px-1 py-0.5 text-[9px]">Enter</kbd> to send · AI responses are illustrative
          </p>
        </div>
      </GlassCard>
    </div>
