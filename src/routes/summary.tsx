...
function ContextAwareSummaryCard({ column, anomaly, index }: { column: ColumnData; anomaly: any; index: number }) {
            ...
            <div>
              ...
              <div className="mt-3 flex items-center justify-between gap-4 text-[9px] font-mono-tight text-muted-foreground uppercase border-t border-border/20 pt-2.5">
                ...
                <div className="flex gap-4 text-foreground font-bold">
                  {categories.map((cat) => (
                    <span key={cat.label}>{cat.label} ({cat.pct}%)</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-primary/10 bg-primary/5 p-4 flex items-center justify-between h-32 gap-6 shadow-[inset_0_0_12px_rgba(16,185,129,0.02)]">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <div className="relative flex items-center justify-center h-16 w-16">
                  ...
                  <span className="absolute font-mono-tight text-[10px] font-bold text-primary">{uniqueness}%</span>
                </div>
                <div>
                  <div className="font-mono-tight text-[9px] uppercase tracking-wider text-muted-foreground">Uniqueness</div>
                  <div className="font-display text-[10px] font-semibold text-primary">Data Quality Ring</div>
                </div>
              </div>
            ...
            <div className="mt-4 rounded-xl border border-primary/10 bg-primary/5 p-4 flex flex-col justify-between h-32 relative overflow-hidden shadow-[inset_0_0_12px_rgba(16,185,129,0.02)]">
              <div className="flex items-center justify-between z-10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono-tight text-[10px] text-primary font-bold uppercase tracking-wider">Row Velocity Curve</span>
                </div>
                <span className="font-mono-tight text-[8px] text-muted-foreground uppercase">Real-time Ingest Activity</span>
              </div>
          ...
          <div className="mt-5 flex h-32 items-end gap-1.5">
            {column.dist.map((v, i) => (
              <motion.div
                ...
                whileInView={{ height: `${v}%` }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 + i * 0.03, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
           ...
           const highCardCount = richCols.filter((c: any) => c.unique > 1000).length;
