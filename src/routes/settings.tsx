...
export const Route = createFileRoute("/settings")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Configure your SchemaSense preferences, notifications, and database connections.",
      },
    ],
  }),
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        <div
          className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg transition-colors ${enabled ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
        >
...
function SettingsPage() {
  ...
  useEffect(() => {
    if (darkMode) {
      ...
      try {
        localStorage.setItem("ss_dark_mode", "1");
      } catch (err) {
        console.warn("Could not set ss_dark_mode:", err);
      }
    } else {
      document.documentElement.classList.remove("dark");
      try {
        localStorage.setItem("ss_dark_mode", "0");
      } catch (err) {
        console.warn("Could not set ss_dark_mode:", err);
      }
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem("ss_voice", voice ? "1" : "0");
    } catch (err) {
      console.warn("Could not set ss_voice:", err);
    }
  }, [voice]);
  ...
  return (
    <div>
      <SectionTitle
        ...
        title={
          <>
            Configure <span className="text-primary">SchemaSense.</span>
          </>
        }
              ...
              {TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    <tab.icon
                      className={`relative z-10 h-4 w-4 flex-shrink-0 transition-colors ${active ? "text-primary" : "text-muted-foreground group-hover:text-foreground"}`}
                    />
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Sets the display language across all SchemaSense modules.
                      </p>
                    </div>
                ...
                <GlassCard className="p-6 md:p-8">
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-background/40 px-4 py-3">
                      ...
                      <span
                        className={`ml-auto inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 font-mono-tight text-[10px] uppercase tracking-wider transition-colors ${
                          connStatus === "ok"
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <p className="text-xs text-muted-foreground">
                        Credentials are stored locally and never transmitted without TLS.
                      </p>
                      <button
