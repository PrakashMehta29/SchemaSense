...
export function determineResponseKey(prompt: string): string {
  ...
  if (query.includes("it") || query.includes("this") || query.includes("that") || query.includes("where is it")) {
...
export function simulateAIService(
  ...
  let stages: ThinkingStage[] = THINKING_STAGES.map((label) => ({ label, status: "pending" }));
